...
- 安裝磁碟映像：`dist/MyDuoBar-1.3.0-universal.dmg`
- 壓縮檔：`dist/MyDuoBar-1.3.0-universal.zip`
...
101 項狀態檢查涵蓋狀態組合、未知狀態、顯示偏好儲存、三種語言的文字、VPN 路由、Wi-Fi 網路分組與動畫觸發。AppKit 檢查涵蓋電量方向、圓點大小、實際圖層狀態變化、按一下動畫、連續更新不中斷旋轉、充電過渡、Wi-Fi 子選單結構，以及在設定視窗切換語言後完整重建畫面。測試不會實際掃描、開關 Wi-Fi 或加入網路。
