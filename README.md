...
狀態列右側的箭頭表示可以前往設定。按一下帶箭頭的整列會收起選單，並直接開啟對應的系統設定頁面：電池或專注模式。Wi-Fi、VPN、耳機與聲音則往右展開子選單，子選單底部可再前往對應的系統設定。沒有 Wi-Fi 硬體的 Mac 按 Wi-Fi 會開啟網路設定。

## 圖示與圓點

- 圖示 32 × 22 pt，能完整放進 22 pt 高的選單列。電池軌道是 28 × 18 pt 的膠囊，線寬 2 pt，底部圓點直徑 2.5 pt。電量降低時從右側開始消退，左端固定。
...
- Wi-Fi 連線、網路名稱、訊號等級或四個狀態改變時，Wi-Fi 圖形保持正向並淡入切換；電量圓弧和圓點沿著膠囊軌道一起滑行一整圈，輕微回彈後歸位，約 1.43 秒結束。滑行由 Core Animation 以疊加動畫合成，期間電量變化與充電掃光照常顯示，連續更新不會重頭播放。按一下開啟選單時也會播放一次。
...
- 安裝磁碟映像：`dist/MyDuoBar-1.6.0-universal.dmg`
- 壓縮檔：`dist/MyDuoBar-1.6.0-universal.zip`
...
101 項狀態檢查涵蓋狀態組合、未知狀態、顯示偏好儲存、三種語言的文字、VPN 路由、Wi-Fi 網路分組與動畫觸發。AppKit 檢查涵蓋電量方向、圓點大小、實際圖層狀態變化、按一下動畫、連續更新不中斷滑行、充電過渡、Wi-Fi 子選單結構、以模擬服務驗證的 VPN 連線／中斷／失敗流程、以模擬裝置驗證的輸出裝置切換、音量滑桿與靜音切換（含快速拖曳合併寫入、無靜音功能與無法調整音量的裝置）、選單開關與滑桿，以及在設定視窗切換語言後完整重建畫面。測試不會實際掃描或開關 Wi-Fi、加入網路、切換任何真實 VPN，也不會改變這台 Mac 的音量。
