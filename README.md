...
- **耳機子選單**：選單中的耳機會往右展開，列出所有輸出裝置，按一下即可切換聲音輸出。
- **聲音子選單**：選單中的聲音會往右展開，可拖曳滑桿調整音量，下方的靜音列可一鍵切換。
...
- 耳機子選單只在使用者選擇裝置時，透過 Core Audio 變更預設的聲音輸出裝置。
- 聲音子選單只在使用者拖曳滑桿或按下靜音時，透過 Core Audio 調整目前輸出裝置的音量或靜音狀態，不錄音、不讀取音訊內容。
...
- 只用 UserDefaults 儲存圓點排列、充電上限檔位等本機偏好。
...
- **輸出裝置**：列出目前可以播放聲音的所有裝置，例如內建喇叭、AirPods、藍牙喇叭、HDMI 或 DisplayPort 螢幕，目前使用中的裝置會打勾。按一下其他裝置即可切換。
- **藍牙設定…**：開啟系統的藍牙設定頁，用來連線尚未連上的耳機。

注意事項：

- 只列出已連線的裝置。已配對但未連線的 AirPods 請先在藍牙設定或 AirPods 上連線。
- macOS 拒絕切換時會開啟聲音設定。
...
狀態列右側的箭頭表示可以前往設定。按一下帶箭頭的整列會收起選單，並直接開啟對應的系統設定頁面：專注模式。Wi-Fi、電池、VPN、耳機與聲音則往右展開子選單，子選單底部可再前往對應的系統設定。沒有 Wi-Fi 硬體的 Mac 按 Wi-Fi 會開啟網路設定。
...
105 項狀態檢查涵蓋狀態組合、未知狀態、顯示偏好儲存、三種語言的文字、充電上限的狀態文字、VPN 路由、Wi-Fi 網路分組與動畫觸發。AppKit 檢查涵蓋電量方向、圓點大小、實際圖層狀態變化、按一下動畫、連續更新不中斷旋轉、充電過渡、Wi-Fi 子選單結構、以模擬服務驗證的 VPN 連線／中斷／失敗流程、以模擬裝置驗證的輸出裝置切換、音量滑桿與靜音切換（含快速拖曳合併寫入、無靜音功能與無法調整音量的裝置）、以模擬服務驗證的充電上限開關與檔位切換（含記住檔位、macOS 拒絕與不支援的情況）、選單開關與滑桿，以及在設定視窗切換語言後完整重建畫面。測試不會實際掃描或開關 Wi-Fi、加入網路、切換任何真實 VPN，也不會改變這台 Mac 的音量或充電上限。
