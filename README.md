...
下载 Release 中的 `DuoBar-0.1.3-universal.dmg`，打开后把 **DuoBar.app 拖进 Applications**，再从“应用程序”中启动。也可以下载 ZIP，解压后把应用移到“应用程序”。
...
- Wi-Fi 连接、网络名称、信号档位或四个状态开关变化时，Wi-Fi 保持正向并淡入切换；电量圆弧和圆点作为完整外圈同步转动，保持相对间距，轻微回弹后归位，约 1.2 秒结束。连续更新不会从头重播，结束后停止动画刷新。
...
- 应用：`build/DuoBar.app`
- 安装磁盘：`dist/DuoBar-0.1.3-universal.dmg`
- 压缩包：`dist/DuoBar-0.1.3-universal.zip`

74 项状态检查覆盖组合、未知状态、显示偏好保存、Wi-Fi 文案、VPN 路由和动画触发。AppKit 检查验证电量方向、圆点大小、外圈同步归位、充电过渡与停止刷新。GitHub Actions 自动运行检查并构建两种架构。
