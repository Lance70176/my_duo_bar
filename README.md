...
下载 Release 中的 `DuoBar-0.1.4-universal.dmg`，打开后把 **DuoBar.app 拖进 Applications**，再从“应用程序”中启动。也可以下载 ZIP，解压后把应用移到“应用程序”。
...
- 电池圆弧与底部圆点采用一致的粗度；电量降低时从右侧开始消退，左端固定。
- 中间显示 Wi-Fi，信号用“信号很好 / 信号一般 / 信号较弱”表达。
- 底部圆点从左到右默认为 **VPN、耳机、静音、专注**。所有圆点等大，直径与电量圆弧线宽一致；开启时点亮，未开启或未知时显示清晰的灰色。
...
- 应用：`build/DuoBar.app`
- 安装磁盘：`dist/DuoBar-0.1.4-universal.dmg`
- 压缩包：`dist/DuoBar-0.1.4-universal.zip`
...
此前实机验收为 Apple Silicon / macOS 26.6.2，包含菜单交互、圆点设置与重启保留、状态读取、系统设置跳转及登录项注册/取消。0.1.4 已验证授权后的专注状态读取、图层状态与点击动画逻辑；新版实际屏幕流畅度和专注开关的完整切换仍待实机验收。实际注销登录、睡眠唤醒、Intel 和其他 macOS 版本尚未验收。
