...
> 本專案是 [n977485865-create/DuoBar](https://github.com/n977485865-create/DuoBar)（commit `b278174`）的個人版本，介面改為繁體中文，並以 macOS 27 為主要目標。

## 與原版的差異

- **名稱**：App 名稱改為 MyDuoBar，Bundle ID 改為 `com.rex.myduobar`，不會與原版共用偏好設定或權限紀錄。
- **語言**：介面、系統權限說明與文件全部改為繁體中文（台灣用語）。
...
- 使用者按下相關按鈕時，會透過系統開啟「系統設定」或瀏覽器，這些 App 可能因此被啟動。
- 只用 UserDefaults 儲存圓點排列等本機偏好。
- 沒有第三方相依套件，執行檔只連結系統框架。
- 唯一的對外動作在設定視窗：按「前往」會用瀏覽器開啟原作者的小紅書頁面；按微信或電子郵件會把文字複製到剪貼簿。
...
第一次啟動會顯示設定視窗。圖示與動畫已包含在 App 中，但系統權限和選單列位置需要在每台 Mac 上分別設定。
...
- 安裝磁碟映像：`dist/MyDuoBar-1.1.0-universal.dmg`
- 壓縮檔：`dist/MyDuoBar-1.1.0-universal.zip`
...
每次建置都從空的 App 目錄開始。打包前會確認 App 只包含主程式、Info.plist、圖示與簽章，並核對兩種架構、簽章與系統函式庫相依性。ZIP 不含 AppleDouble 附加檔案，測試、診斷腳本與本機偏好不會進入安裝包。

74 項狀態檢查涵蓋狀態組合、未知狀態、顯示偏好儲存、Wi-Fi 文字、VPN 路由與動畫觸發。AppKit 檢查涵蓋電量方向、圓點大小、實際圖層狀態變化、按一下動畫、連續更新不中斷旋轉，以及充電過渡。

## 原作者

- [小紅書](https://www.xiaohongshu.com/user/profile/5fd62d06000000000101e8b1)
- 微信：**nybbamboo**
- 電子郵件：[nybbamboo@163.com](mailto:nybbamboo@163.com)

## 已知問題：選單列開關已開啟但圖示不顯示

原作者在 macOS 26.6.2 遇過 DuoBar 的選單列開關已開啟，圖示卻被控制中心擋住的情況。實際測試發現，關閉 ChatGPT 的選單列開關會連帶隱藏 DuoBar，重新開啟後恢復。App 本身不依賴 ChatGPT，這是 macOS 把選單列項目錯誤關聯到啟動來源的問題。
