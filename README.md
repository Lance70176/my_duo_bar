...
- **聲音子選單**：選單中的聲音會往右展開，可拖曳滑桿調整音量、一鍵靜音，下方列出所有輸出裝置，按一下即可切換聲音輸出。
...
- 聲音子選單只在使用者拖曳滑桿、按下靜音或選擇輸出裝置時，透過 Core Audio 調整目前輸出裝置的音量或靜音狀態，或變更預設的聲音輸出裝置；不錄音、不讀取音訊內容。
...
- 輸出裝置只列出已連線的裝置。已配對但未連線的 AirPods 請先在藍牙子選單連線。
- macOS 拒絕切換輸出裝置時會開啟聲音設定。
...
101 項狀態檢查涵蓋狀態組合、未知狀態、顯示偏好儲存、三種語言的文字、VPN 路由、Wi-Fi 網路分組與動畫觸發。AppKit 檢查涵蓋電量方向、圓點大小、實際圖層狀態變化、按一下動畫、連續更新不中斷旋轉、充電過渡、Wi-Fi 子選單結構、以模擬服務驗證的 VPN 連線／中斷／失敗流程、以模擬裝置驗證的藍牙連線／中斷／拒絕流程與輸出裝置切換、音量滑桿與靜音切換（含快速拖曳合併寫入、無靜音功能與無法調整音量的裝置）、選單開關與滑桿，以及在設定視窗切換語言後完整重建畫面。測試不會實際掃描或開關 Wi-Fi、加入網路、切換任何真實 VPN、連線或中斷任何藍牙裝置，也不會改變這台 Mac 的音量或輸出裝置。
